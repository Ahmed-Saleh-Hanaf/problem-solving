...
{
    ll n,q; cin >> n >> q;
    vc<vc<ll>> rec(1002, vc<ll>(1002));
    while(n--)
    {
        ll h, w ; cin >> h >> w;
        rec[h][w]+= h*w;
    }
    loop(i,1,1001)loop(j,1,1001) rec[i][j]+= rec[i-1][j] + rec[i][j-1] - rec[i-1][j-1];

    while(q--)
    {
        ll hs,ws,he,we; cin >> hs >> ws >> he >> we;
        hs++;ws++;
        he--;we--;
        ll ans = rec[he][we]-rec[hs-1][we]-rec[he][ws-1]+rec[hs-1][ws-1];
        cout << ans << el;
    }
